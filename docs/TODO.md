...
- [ ] **Bearbeitungshistorie refinements** (per-task evaluation history feature shipped) — Use a smaller header, make the section collapsible (default open or closed to be decided), and surface history entries inside the assignment viewer too (currently only in the per-task detailed view).
