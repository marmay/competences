...
import Competences.Document.Competence (Competence (..), CompetenceLevelId, Level)
...
import Competences.Document.Order (orderMin)
...
matchExchangeDoc doc xdoc =
  let tps = map (\t -> TaskPreview t False (matchExchangeTask doc False t)) xdoc.tasks
...
matchExchangeAssignment doc isDraft a =
  let existing = findAssignmentByName doc isDraft a.name
...
matchExchangeTask doc isDraft t =
  let existing = findTaskByIdentifier doc isDraft (TaskIdentifier t.identifier)
...
matchExchangeResource doc r =
  let existing = findResourceByIdentifier doc r.identifier
...
matchExchangeLesson doc l =
  let existing = findLessonByTitle doc l.title
...
fromExchangeContent = \case
  ...
  ExFileContent _ ->
    -- File-backed resources need attachment resolution we don't have
    -- yet; leave content empty and surface the missing file in apply.
    InlineContent (fromTrustedInput "")

resourceEquals :: Resource -> Resource -> Bool
resourceEquals a b =
  a.identifier == b.identifier && a.content == b.content
