...
handleAssignmentsCommand :: CommandContext -> AssignmentsCommand -> Document -> UpdateResult
handleAssignmentsCommand cmdCtx (OnAssignments c) d = case c of
  Delete aid -> do
    validateAssignmentNotReferenced d aid
    (d', a) <- assignmentContext.delete aid d
    pure (d', assignmentContext.affectedUsers a d)
  _ ->
    interpretEntityCommand assignmentContext cmdCtx c d
  where
    assignmentContext =
