...
saveCommandWithAudience :: Pool Connection -> CommandId -> UserId -> Command -> CommandAudience -> IO Int64
saveCommandWithAudience pool cmdId userId cmd audience = withResource pool $ \conn -> do
  let envelope = wrapCommand userId cmd
  let envelopeJson = encode envelope
