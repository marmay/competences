...
saveCommand :: Pool Connection -> UserId -> SessionId -> Command -> IO (CommandId, Int64)
saveCommand pool userId sessionId cmd = withResource pool $ \conn -> do
  commandId <- UUID.nextRandom
  let envelope = wrapCommand userId sessionId cmd
...
saveCommandWithAudience :: Pool Connection -> CommandId -> UserId -> SessionId -> Command -> CommandAudience -> IO Int64
saveCommandWithAudience pool cmdId userId sessionId cmd audience = withResource pool $ \conn -> do
  let envelope = wrapCommand userId sessionId cmd
  let envelopeJson = encode envelope
  ...
  [Only generation] <-
    query
      ...
      RETURNING generation
    |]
      (cmdId.unId, userId.unId, envelopeJson, audienceText)
